...
```
T_end (0s):    📬 Уведомление 1 → ВИБРАЦИЯ
               ⚠️ Timer работает, но play() заблокирован в AOD
               
T_end+5s:      📬 Уведомление 2 → ВИБРАЦИЯ
               ⚠️ Timer работает, но play() заблокирован в AOD
               
T_end+10s:     📬 Уведомление 3 → ВИБРАЦИЯ
               → Экран "просыпается" для показа уведомления
               → Timer "ловит" момент активного экрана
               → play() РАБОТАЕТ!
               
T_end+11s:     📳 Timer → ВИБРАЦИЯ (каждую секунду)
T_end+12s:     📳 Timer → ВИБРАЦИЯ
T_end+13s:     📳 Timer → ВИБРАЦИЯ
...            📳 непрерывно до подтверждения пользователем
```

**Итог для пользователя:**
- 3 вибрации с паузами 5 секунд (от уведомлений)
- Сразу после 3-й — вибрации каждую 1 секунду (от Timer)
- Рука НЕ поднималась!

**Почему именно после 3-го уведомления:**
- Каждое уведомление "будит" экран на короткое время
- Первые два: экран быстро засыпает, Timer не успевает "поймать" момент
- Третье: Timer "ловит" момент активного экрана, начинает haptic
- Каждый haptic поддерживает экран активным → непрерывный цикл
