...
## 🧪 Тест AOD режима (2026-01-08)

### Поведение при wrist-down (экран AOD):

```
сигнал — пауза 5с — сигнал — пауза 5с — сигнал — [поднял руку] — сигналы без перерыва
   │                   │                   │                            │
notification.end   notification.end.2   notification.end.3    WKInterfaceDevice.play()
   (T+0)              (T+5s)              (T+10s)               каждую 1 сек
```

### Объяснение:

| Режим | `WKInterfaceDevice.play()` | Local Notifications | Результат |
|-------|---------------------------|---------------------|-----------|
| **wrist-down (AOD)** | ❌ Заблокирован системой | ✅ Работают | 3 вибрации с интервалом 5 сек |
| **wrist-up (активен)** | ✅ Работает | ✅ Работают | Непрерывный haptic каждую 1 сек |

### Вывод:

**Это ОЖИДАЕМОЕ и ПРАВИЛЬНОЕ поведение!**

- В AOD режиме watchOS блокирует прямой haptic для экономии батареи
- Системные уведомления — единственный способ дать вибрацию в AOD
- Когда пользователь поднимает руку, приложение активируется и даёт непрерывный haptic
