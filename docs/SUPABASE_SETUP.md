...
> ⚠️ **ВАЖНО**: Supabase используется **только временно** для тестирования и разработки.  
> После активации Apple Developer Account планируется **миграция на CloudKit**.  
> См. [PRODUCTION_RELEASE.md](PRODUCTION_RELEASE.md) для деталей миграции.

---

## 🎯 Назначение

Этот документ описывает настройку Supabase как **временного решения** для backend:
- ✅ Тестирование сохранения и загрузки медитаций
- ✅ Разработка без необходимости Apple Developer Account
- ✅ Быстрый старт для MVP

**После активации Apple Developer Account:**
- 🔄 Миграция на CloudKit (нативное решение Apple)
- 🔄 Удаление Supabase зависимостей
- 🔄 Использование CloudKit для синхронизации и хранения
...
**⚠️ Важно**: Это временное решение для разработки. После миграции на CloudKit:
- CloudKit автоматически обрабатывает авторизацию через iCloud
- Не нужны временные policies или foreign key constraints
- Данные синхронизируются автоматически между устройствами

---

## 📋 Шаг 5: Настройка Apple Sign In в Supabase (ОПЦИОНАЛЬНО)

> ⚠️ **Примечание**: Apple Sign In в Supabase настраивать **не обязательно**, так как планируется миграция на CloudKit, который использует встроенную авторизацию через iCloud.

Если вы хотите протестировать авторизацию с Supabase:
...
- [ ] Apple Sign In настроен (опционально, не обязательно для MVP)

---

## 🔄 Миграция на CloudKit

После активации Apple Developer Account:

1. **Настроить CloudKit Container** в Xcode
2. **Создать CloudKit Schema** (Meditation record type)
3. **Заменить SupabaseService на CloudKitService**
4. **Удалить Supabase зависимости** из проекта
5. **Удалить Config.swift** (больше не нужен)
6. **Удалить временные Supabase policies** (если были созданы)

📖 **Подробная инструкция**: [PRODUCTION_RELEASE.md](PRODUCTION_RELEASE.md) → "Этап 1.5: Миграция на CloudKit"

---

## ⚠️ Ограничения Supabase Free Tier

- **Автопауза через 7 дней неактивности** - проект приостанавливается
- **Ограниченное хранилище** - для MVP достаточно, но для продакшена лучше CloudKit
- **Требует внешний сервис** - CloudKit интегрирован в Apple экосистему

**Рекомендация**: Используйте Supabase только для разработки и тестирования. Для продакшена переходите на CloudKit.

---

**Готово!** Теперь приложение может сохранять медитации в Supabase (временно). 🎉
...
- [ARCHITECTURE_CURRENT.md](ARCHITECTURE_CURRENT.md) - Текущая архитектура
